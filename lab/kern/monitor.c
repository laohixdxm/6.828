...
static struct Command commands[] = {
  ...
  { "mon_backtrace", "Display the current stack trace", mon_backtrace },
};
...
mon_backtrace(int argc, char **argv, struct Trapframe *tf)
{
  ...
  char *fmt = "ebp %08x eip %08x args %08x %08x %08x %08x %08x\n";
  ...
  while (current_ebp != 0) {
    ...
    for (i = 0; i < max_num_args; i++) {
      args[i] = read_byte_at_addr((uint32_t *) (current_ebp - (i + 2) * sizeof(uint32_t)));
    }

    cprintf(fmt, current_ebp, saved_eip, args[0], args[1], args[2], args[3], args[4]);
